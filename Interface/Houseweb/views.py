...
def AdjustGraph(request):
    ...
    for o in range(len(box_order)):
        room.append(float((rooms[int(float(box_order[o])) - 1])))
    boxes_end = []
    for i in range(len(box_order)):
        tmp = []
        for j in range(4):
            tmp.append(float(boxes_pred[int(float(box_order[i])) - 1][j]))
        boxes_end.append(tmp)
    
    data_js['roomret'] = []
    for k in range(len(room)):
        data = boxes_end[k], [mdul.room_label[int(room[k])][1]], int(box_order[k]) - 1
...
def Save_Editbox(request):
    ...
    # Pre-process inputs from Matlab format to Numpy/List
    # Views.py has them as lists or matlab.double?
    # boundary_mat = matlab.double(boundary) -> Python list/array?
    # boundary is just list/array in python usually.
    # In Save_Editbox: 
    # boundary=[[float(x)...]...] list of lists.
    # rType is tensor? No, fp_end.get_rooms returns tensor, converted to list?
    # rType = fp_end.get_rooms(tensor=False) -> numpy array? or tensor?
    # rType_mat = matlab.double(rType.tolist())
    
    # Just pass the python objects directly, align_fp handles conversion.
    box_out, box_order, rBoundary = align_fp(boundary, Box, rType, Edge, fp=None, threshold=18.0)
    
    # Adapt outputs
    box_refine = [box_out, box_order, rBoundary]
